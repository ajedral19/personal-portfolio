	...
	const grabContents = () => {
		...
		projects.map((project) => {
			if (project.id === id) proj = project;
				...
				let steps = 1;
				if (project.category === category && projs.length < 8) projs.push(project);
			}
		});
							...
							<div className={cn("col-sm-4", "col-md-3", "col-lg-5")}>
								<div>
									<div className={cn(style.image_wrap)}>
										{/* <Image src="" /> */}
										<Image layout="fill" objectFit="cover" src={prj_img} alt="this is test image" />
									</div>
								</div>
							</div>
							<div className={cn("col-sm-4", "col-md-3", "col-lg-7")}>
								<div className={cn("t-left", style.content)}>
									<Description title={state.project.title} description={state.project.description} classes={["mb-2"]}>
										{state.project.tags && (
								...
								{state.other_projects ? (
									<div className={cn("mt-2")}>
										<span className={cn("block", "t-left", "mb-1", "medium")}>Check my other projects</span>

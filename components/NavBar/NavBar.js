			...
			>
				<div className={cn({ container }, style.container)}>
					<div className={cn(style.nav_row)}>
						...
						<button className={cn(style.nav_btn, style.nav_menu_toggle_btn, { [style.show]: state.show_menu })} onClick={() => setState({ ...state, show_menu: !state.show_menu })}>
							<span>
								<svg width="24" height="17" viewBox="0 0 24 17">
									<g id="Component_2_2" transform="translate(0 1.5)">
										<line id="line_1" className="line1" x1="24" fill="none" stroke="currentColor" strokeWidth="3" />
										<line id="line_2" className="line2" x1="24" transform="translate(0 7)" fill="none" stroke="currentColor" strokeWidth="3" />
										<line id="line_3" className="line3" x1="16" transform="translate(0 14)" fill="none" stroke="currentColor" strokeWidth="3" />
									</g>
								</svg>
							</span>
						</button>
						...
						<button
							...
							onClick={() => switchTheme(state, setState)}
						>
							<span>
						...
						>
							...
							>
								<li className={cn(style.menu_item)}>
									<a onClick={() => navigateTo(router, "resume")}>Portfolio</a>
								</li>

								{state.transform_menu && (
									<li className={cn(style.menu_item, style.footer)}>
										<Link href="/">
											<a className={cn("medium", "tiny")}>GoGoGoPowerAJ &copy;</a>
										</Link>
									</li>
								)}
							</ul>
